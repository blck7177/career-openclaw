...
import { UserCircle, Plus, Briefcase } from "lucide-react";
...
function ProfileCard({ profile }: { profile: CandidateProfile }) {
  ...
  return (
    <Link href={`/profile/${profile.candidate_profile_id}`} className="block">
        ...
        <CardContent className="pt-5 pb-5">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right text-xs text-muted-foreground shrink-0">
              ...
              <p className="mt-1 font-mono">{profile.candidate_profile_id.slice(0, 12)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
